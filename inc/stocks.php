...
function tbs_auto_paragraphh($text){
    if (trim($text) !== '') {
...
function tbs_clear_phonee ($phone) {
    return str_replace(array(' ', '(', ')', '-'), '', str_replace('+7', '8', $phone));
}
function tbs_text_sliderr ($text, $count_words = 57, $class = 'slick--slider') {
...
function tbs_video_style_js_initt ($selector = '#youtubelist') {
    ?>
...
function tbs_rand_indexess ($min, $max, $count_) {
    $count = $count_;
...
function tbs_list_of_catt ($term_name) {
    global $wpdb;
...
function tbs_list_post_by_post_typee ($post_type, $cat_id = false) {
    global $wpdb;
...
function tbs_get_best_stocks () {
    ...
    return $wpdb->get_results(
        "SELECT * FROM `$wpdb->posts` as posts INNER JOIN
             ...
             WHERE 
                posts.`post_type` = 'stocks'
                AND posts.`post_status` = 'publish'
                   AND postmeta.`meta_key` = 'cdistocks-meta-best'
...
function tbs_get_id_by_urll ($url) {
    global $wpdb;
...
function tbs_get_number_with_zeroo ($number) {
...
function tbs_replace_str_with_starr ($str) {
